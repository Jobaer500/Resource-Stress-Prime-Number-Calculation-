...
Includes a minimal HTTP health check server on port 8080 for container
orchestration platforms (ECS, Kubernetes, etc.) to verify the service
is running. The load generators run independently in the background.
...
class HealthCheckHandler(http.server.BaseHTTPRequestHandler):
    """Simple HTTP handler that returns 200 OK for any GET request."""
...
def run_health_check_server(port: int, stop_event):
    """Start an HTTP server on the given port for health checks."""
    try:
...
def main():
    ...
    health_check_thread = threading.Thread(
        ...
        args=(health_check_port, health_check_event),
        daemon=True,
        name="HealthCheckThread"
    )
    health_check_thread.start()
    threads.append(health_check_thread)
    thread_stop_events.append(health_check_event)
